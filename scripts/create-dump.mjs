...
if (requestedGalleries.length === 0 || requestedGalleries.some((gallery) => !validGalleries.has(gallery))) {
  throw new Error(`Gallery must be one or more of: ${Array.from(validGalleries).join(", ")}.`);
}

if (new Set(requestedGalleries).size !== requestedGalleries.length) {
  throw new Error("Gallery IDs must be unique.");
}

const galleries = ["home", ...requestedGalleries.filter((gallery) => gallery !== "home")];
